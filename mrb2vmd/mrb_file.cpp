...
/**
 * MRBファイル内(Little Endian)のデータを32bit整数にして返す。
 *
 * @param d バイト列(4byte以上ある前提。間違ってるとバッファオーバーランするので注意)
 * @return 32bit値
 */
static uint32_t
...
/**
 * MRBファイルを読み込み、指定された構造体に格納する。
 *
 * @param path MRBファイルパス
 * @param mrb MRBデータを格納する構造体。
...
/**
 * MRBファイル内の１つのデータセクションを読み込む。
 *
 * @note メモ
 *       ファイル構造が想定どおりであることを前提にしてるなあ。
 * 
 * @param fp ファイル
 * @param data 格納するデータの構造体
...
read_mrb_data(FILE *fp, struct MRBData *data)
{
    ...
    /* TODO : 汎用的に読み込むならここに各タイプの読み出しを追加する。 */
    case MRB_TYPE_BONE:
